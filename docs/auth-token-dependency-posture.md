...
| RFC 8693 token exchange client | `crates/mcp-toolkit-auth/src/outbound_dpop.rs` | Crate-backed HTTP, JOSE, URL, and typed local glue | Use the no-redirect/no-ambient-proxy client, mandatory audit metadata, endpoint-keyed bounded nonce stores, one nonce retry, fixed access-token-for-access-token identifiers, exact HTTP 200 JSON success handling, and fail-closed `token_type=DPoP` plus access-token `issued_token_type` validation documented in `docs/design/outbound-dpop-token-exchange.md`. |
