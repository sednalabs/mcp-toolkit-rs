...
#[cfg(test)]
mod tests {
    use super::{ToolListPageEvidence, assert_complete_tool_catalogue};
    use serde_json::json;
    ...
    fn rejects_first_page_only_evidence() {
        ...
        assert!(
            catch_unwind(|| assert_complete_tool_catalogue(&pages, 2, &["sentinel"])).is_err()
        );
    }
    ...
    fn rejects_cursor_cycles_and_duplicate_tool_names() {
        ...
        assert!(
            catch_unwind(|| assert_complete_tool_catalogue(&cursor_cycle, 2, &["second"]))
                .is_err()
        );
        assert!(
    ...
    fn rejects_broken_cursor_chain_and_missing_sentinel() {
        ...
        assert!(
            catch_unwind(|| assert_complete_tool_catalogue(&broken_chain, 2, &["second"]))
                .is_err()
        );
        assert!(
            catch_unwind(|| assert_complete_tool_catalogue(&missing_sentinel, 1, &["sentinel"]))
                .is_err()
        );
    }
}
