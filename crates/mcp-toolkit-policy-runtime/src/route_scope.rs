...
impl RouteScopePolicyConfig {
    ...
    {
        Self {
            allowed_paths: normalize_paths(paths),
...
fn normalize_paths<I>(values: I) -> Vec<String>
where
    I: IntoIterator,
    I::Item: AsRef<str>,
{
...
mod tests {
    ...
    #[test]
    fn normalizes_configured_paths_with_request_paths() {
        let config = RouteScopePolicyConfig::new(
            ["mcp", "/mcp/", " //mcp// "],
            ["POST"],
            Vec::<String>::new(),
        );
        assert_eq!(config.allowed_paths, vec!["/mcp"]);

        let request = RouteScopePolicyRequest::new("POST", "mcp/", true, Vec::<String>::new());
        assert!(evaluate_route_scope(&config, &request).allow);
    }

    #[test]
    fn blank_configured_paths_do_not_authorize_root() {
        let blank = RouteScopePolicyConfig::new([""], ["POST"], Vec::<String>::new());
        assert!(blank.allowed_paths.is_empty());
        let root = RouteScopePolicyRequest::new("POST", "/", true, Vec::<String>::new());
        assert_eq!(
            evaluate_route_scope(&blank, &root).code.as_deref(),
            Some("INVALID_PATH")
        );

        let mixed = RouteScopePolicyConfig::new(["/mcp", ""], ["POST"], Vec::<String>::new());
        assert_eq!(mixed.allowed_paths, vec!["/mcp"]);
        assert_eq!(
            evaluate_route_scope(&mixed, &root).code.as_deref(),
            Some("INVALID_PATH")
        );
    }
