...
impl Default for ListDrainLimits {
    fn default() -> Self {
        Self::new(
            DEFAULT_LIST_DRAIN_MAX_PAGES,
            DEFAULT_LIST_DRAIN_MAX_ITEMS,
        )
    }
}
...
mod tests {
    use super::{
        FetchedListPage, ListDrainError, ListDrainLimits, PaginationError,
        collect_paginated_list, paginate_list,
    };
    use rmcp::model::PaginatedRequestParams;
    ...
    fn client_drain_rejects_cursor_cycle_without_partial_success() {
        ...
        assert_eq!(
            result,
            Err(ListDrainError::RepeatedCursor("a".to_string()))
        );
    }
