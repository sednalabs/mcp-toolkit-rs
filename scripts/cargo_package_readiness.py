...
def main() -> int:
    ...
    path_names = [package.manifest_path.parent.name for package in packages]
    if path_names != FIRST_WAVE:
        print(
            f"First-wave manifest path/order mismatch. expected={FIRST_WAVE} observed={path_names}",
            file=sys.stderr,
        )
        return 1
