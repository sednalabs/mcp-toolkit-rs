...
fn stdio_initializes_and_lists_tools() {
    assert_stdio_tools_list(
        env!("CARGO_BIN_EXE_single-crate-public-stdio-server"),
...
fn cli_print_tools_lists_active_profile_tools() {
    let output = Command::new(env!("CARGO_BIN_EXE_single-crate-public-stdio-server"))
        .arg("--print-tools")
...
fn cli_doctor_reports_generated_project_ready() {
    ...
    assert!(stdout.contains("Shape: public-stdio"));
...
fn cli_print_client_config_renders_stdio_config() {
    ...
    let stdout = String::from_utf8(output.stdout).expect("utf8 stdout");
    assert!(stdout.contains("[mcp_servers.\"single-crate-public-stdio-server\"]"));
    assert!(stdout.contains("command = \""));
    assert!(stdout.contains("SINGLE_CRATE_PUBLIC_STDIO_SERVER_TOOL_PROFILE = \"read_only\""));
}
